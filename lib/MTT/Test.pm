            ...
            foreach my $install_key (keys(%{$version})) {
                ...
                foreach my $build_key (keys(%{$install})) {
                    ...
                    foreach my $run_key (keys(%{$build})) {
						...
						#if (defined(MTT::Util::does_hash_key_exist($MTT::Test::builds, qw/$get_key $version_key  $install_key $build_key/))) 
                        if (defined(MTT::Util::does_hash_key_exist($MTT::Test::builds, $get_key, $version_key,  $install_key, $build_key))) 
						{
