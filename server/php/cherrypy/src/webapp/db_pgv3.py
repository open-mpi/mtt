...
class DatabaseV3():
    ...
    def _detail(self, phase, columns, search, options=[]):
        ...
        select_stmt =  "SELECT %s \n" % (", ".join(legal_columns))
        select_stmt += "FROM %s \n" % (phase_name)
        select_stmt += "%s \n" % (" \n".join(["INNER JOIN %s ON %s.%s = %s.%s " % (t,t,TABLE_TREE[t]["key"], self._select_item_in(TABLE_TREE[t]["parents"], [phase_name] + table_order[:i]), TABLE_TREE[t]["foreign_key"]) for i,t in enumerate(table_order)]))
        select_stmt += "WHERE %s \n" % ("\nAND ".join(["%s = '%s'" % (k,v) for k,v in legal_search_columnnames.items()]))
